...
function showFatalError(error) {
  ...
  $('#ranking-body').innerHTML = '<tr><td class="empty-state" colspan="8"><span>无法读取最新快照。已发布的历史数据不会因此被改写。</span></td></tr>';
...
async function init() {
  ...
  function renderRanking() {
    ...
    const items = data.rankings.current.filter(item => {
      const haystack = `${item.repo} ${item.description} ${item.category}`.toLowerCase();
    ...
    $('#ranking-body').innerHTML = items.length ? items.map(item => `
      ...
      </tr>`).join('') : '<tr><td class="empty-state" colspan="8"><span>没有匹配项目。清空搜索词或选择“全部分类”后可恢复完整排名。</span></td></tr>';
  }
